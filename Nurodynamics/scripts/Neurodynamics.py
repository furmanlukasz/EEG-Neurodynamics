...
class ComputeTask(object):
    ...
    def __init__(self, subject,data,srate,electrode):
        ...
        self.data = data
        self.eegData = self.data

        self.srate = srate
        self.electrode = electrode
        self.el_index = self.electrode
        self.subject = subject
        #self.interval = len(self.eegData)
        #self.timestep = self.interval / self.srate
        self.eegSample = self.eegData
        self.eegSampleRaw = self.eegSample
        self.time = np.arange(len(self.eegSample))/self.srate

        self.regular_info_title = 'Electrode: '
    ...
    def computeRP(self,td,emb,metric, nbr=' '):
        self.nbr = nbr
        self.timedelay = td
        self.embedding = emb
        self.metric = metric
        ...
        settings = Settings(time_series,
                            ...
                            # eval(self.metric),  # Sigmoid,#TaxicabMetric,#Sigmoid,#EuclideanMetric,
                            theiler_corrector=1)
        if nbr != ' ':
            settings = Settings(time_series,
                                analysis_type=Classic,
        ...
        computation1 = RPComputation.create(settings,
                                            verbose=True)
        result1 = computation1.run()
